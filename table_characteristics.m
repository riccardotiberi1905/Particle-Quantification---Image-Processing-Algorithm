function [stats_roi, n_thrombi, stats_below_500, stats_500, stats_800, stats_1000, major_axis_length] = table_characteristics(negative_binary_image, negative_roi_binary_image)
    %{
    ...
    label_roi = bwlabel(negative_roi_binary_image);
    n_thrombi_roi = max(max(label_roi));
    label_filt_roi = bwareafilt(negative_roi_binary_image, n_thrombi_roi);
    ...
    % We remove all thrombi with a MAL < 100 um
    particles_to_delete = stats_roi.MajorAxisLength < 100;
